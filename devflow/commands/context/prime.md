...
- If directory doesn't exist or is empty:
  ...
  - Exit gracefully
- Count available context files: `ls -1 devflow/context/*.md 2>/dev/null | wc -l`
- Report: "Found {count} context files to load"

### 2. File Integrity Check
- For each context file found:
  - Verify file is readable: `test -r "devflow/context/{file}" && echo "readable"`
  - Check file has content: `test -s "devflow/context/{file}" && echo "has content"`
  - Check for valid frontmatter (should start with `---`)
- Report any issues:
  - Empty files: "{filename} is empty (skipping)"
  - Unreadable files: "Cannot read {filename} (permission issue)"
